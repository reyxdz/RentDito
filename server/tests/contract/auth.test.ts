...
afterAll(async () => {
  // The "authenticates a protected route" test above makes a real
  // authenticated request through the full app, and server.ts's
  // `app.use(auditLog)` middleware writes an `audit_logs` row for every
  // authenticated mutating request. `audit_logs.user_id -> profiles.id` is
  // `ON DELETE SetNull` (deleting the profile just nulls the actor link,
  // it no longer blocks the delete), but this suite must still leave zero
  // rows behind for these throwaway users -- explicitly delete the audit
  // rows too, rather than relying on the FK to clean them up, since it no
  // longer does.
