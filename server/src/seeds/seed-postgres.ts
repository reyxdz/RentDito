 ...
 * gets `legacyMongoId` set to the _id of the matching Mongo user (see
 * LEGACY_MONGO_IDS_BY_EMAIL below), matched by the stable seed emails, kept
 * as a rollback aid now that MongoDB itself has been removed from the
 * codebase (task 31b).
 *
...
// =============================================================================
// 2. legacy_mongo_id per email -- FROZEN LITERALS, not a live lookup.
//
// Until task 31b, this was resolved live from a running MongoDB (mongoose
// connecting to MONGO_URI and reading the `users` collection populated by the
// now-deleted src/seeds/seed.ts). That connection was removed along with the
// rest of the MongoDB layer -- the codebase will never seed or query Mongo
// again, so there is nothing left to look these ids up FROM.
//
// These 14 hex strings are the exact, real MongoDB `_id` values the old
// Mongo seed (seed.ts) produced for these 14 emails, captured one final time
// on 2026-09-01 (task 31b) while the legacy MongoDB instance was still up and
// still held the pre-migration data, by running seed.ts's user-creation step
// and then reading `db.users.findOne({ email })._id` for each seed email.
// Freezing them is safe specifically BECAUSE MongoDB is being deleted here:
// nothing will ever reseed Mongo differently, so these values can never go
// stale relative to "the" Mongo database -- there is only ever one snapshot
// of it, and this is it. `landlord1@rentdito.com`'s id
// (`6a93e154107b82fb635d8fde`) is the one exception that was ALREADY a fixed
// literal in seed.ts itself (pinned there because contract.service.ts's
// `reviewedBy` field bakes that exact hex string into a golden fixture); the
// other 13 were Mongo-driver-autogenerated and are transcribed verbatim here
// for the first and only time.
//
// Do not "clean up" these magic hex strings -- they are not placeholders,
// they are the actual historical Mongo identity of each seed user, kept only
// so `profiles.legacy_mongo_id` (a deliberate rollback aid, see
// prisma/schema.prisma) keeps being populated with real values instead of
// silently going null once MongoDB is gone.
// =============================================================================

const LEGACY_MONGO_IDS_BY_EMAIL: Record<string, string> = {
  'admin@rentdito.com': '6a96339b76f146dc711a21ab',
  'landlord1@rentdito.com': '6a93e154107b82fb635d8fde',
  'landlord2@rentdito.com': '6a96339b76f146dc711a21ad',
  'manager@rentdito.com': '6a96339b76f146dc711a21ae',
  'maintenance@rentdito.com': '6a96339b76f146dc711a21af',
  'finance@rentdito.com': '6a96339b76f146dc711a21b0',
  'receptionist@rentdito.com': '6a96339b76f146dc711a21b1',
  'staff@rentdito.com': '6a96339b76f146dc711a21b2',
  'user1@rentdito.com': '6a96339b76f146dc711a21b3',
  'user2@rentdito.com': '6a96339b76f146dc711a21b4',
  'user3@rentdito.com': '6a96339b76f146dc711a21b5',
  'user4@rentdito.com': '6a96339b76f146dc711a21b6',
  'user5@rentdito.com': '6a96339b76f146dc711a21b7',
  'user6@rentdito.com': '6a96339b76f146dc711a21b8',
};

function resolveLegacyMongoIds(): Record<string, string> {
  const map: Record<string, string> = {};
  for (const email of SEED_EMAILS) {
    const id = LEGACY_MONGO_IDS_BY_EMAIL[email];
    if (!id) {
      throw new Error(`FATAL: no frozen legacy Mongo id literal for email "${email}" -- add it to LEGACY_MONGO_IDS_BY_EMAIL.`);
    }
    map[email] = id;
  }
  return map;
...
async function run() {
  ...
  const legacyIds = resolveLegacyMongoIds();
  console.log(`Resolved ${Object.keys(legacyIds).length} legacy Mongo user ids (frozen literals).`);
