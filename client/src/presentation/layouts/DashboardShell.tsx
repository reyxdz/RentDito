...
import { renderIcon } from '../../application/config/menuConfig';
...
export interface DashboardShellProps {
  /** Title shown next to logo in the sidebar */
  brandTitle: string;
  /** Fallback text for the AppBar when no menu item matches the current path */
  appBarFallback: string;
  /** Pre-filtered list of menu items to render in the sidebar */
  menuItems: MenuItemType[];
  /** Whether to render section label dividers above grouped items (default: true) */
  showSectionLabels?: boolean;
  /** Configuration for the sidebar profile footer */
  profileConfig: {
    subtitle?: string;
    chipLabel?: string;
...
export default function DashboardShell({
  ...
  const renderNavItem = (item: MenuItemType, idx: number) => {
    const isActive =
      ...
      (item.path !== '/hub' && item.path !== '/admin' && item.path !== '/u' &&
        location.pathname.startsWith(item.path));

    const shouldShowLabel = showSectionLabels && item.sectionLabel && !(isCollapsed && isMdUp);
  ...
  const drawer = (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflowX: 'hidden' }}>
      ...
      <SidebarProfile
        ...
        isMdUp={isMdUp}
        subtitle={profileConfig.subtitle}
        chipLabel={profileConfig.chipLabel}
        chipColor={profileConfig.chipColor}
      />
    </Box>
  );
      ...
      >
        <Toolbar>
          ...
          <Typography variant="h6" noWrap component="div" sx={{ flexGrow: 1, color: 'text.primary', fontWeight: 600 }}>
            {menuItems.find(m => m.path === location.pathname)?.text || appBarFallback}
          </Typography>
