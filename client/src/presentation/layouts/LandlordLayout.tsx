...
import { 
  Menu as MenuIcon, Dashboard, HomeWork, AccountBalanceWallet, 
  MeetingRoom, EventNote, SupportAgent, Handyman,
  Brightness4, Brightness7, Logout, ChevronLeft, ChevronRight
...
const MENU_ITEMS = [
  { text: 'Overview', icon: <Dashboard />, path: '/landlord' },
  { text: 'Properties', icon: <HomeWork />, path: '/landlord/properties' },
  { text: 'Units & Rooms', icon: <MeetingRoom />, path: '/landlord/units' },
  { text: 'Bookings', icon: <EventNote />, path: '/landlord/bookings' },
  { text: 'Financials', icon: <AccountBalanceWallet />, path: '/landlord/financials' },
  { text: 'Agents', icon: <SupportAgent />, path: '/landlord/agents' },
  { text: 'Maintenance', icon: <Handyman />, path: '/landlord/maintenance' },
];
      ...
      <List sx={{ px: isCollapsed && isSmUp ? 1 : 2, pt: 2, flexGrow: 1 }}>
        {MENU_ITEMS.map((item) => {
          const isActive = location.pathname === item.path || 
                           (item.path !== '/landlord' && location.pathname.startsWith(item.path));
